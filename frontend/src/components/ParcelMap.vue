...
async function initMap() {
  ...
  if (parcel.geometry) {
    drawPolygon(parcel.geometry, '#fff', 0.1, 2)
...
function addMapLabel(html: string, position: google.maps.LatLngLiteral) {
  if (!map) return
  ...
  class MapLabel extends google.maps.OverlayView {
    ...
    constructor(d: HTMLDivElement, pos: google.maps.LatLngLiteral) {
      super()
      this.div = d
  ...
  const label = new MapLabel(div, position)
  label.setMap(map)
}
...
<template>
  <div class="parcel-map-container">
    ...
    <div class="map-disclaimer">
      Parcel and building boundaries are approximate. Source data from LA County GIS may not align precisely with satellite imagery.
    </div>
  </div>
</template>
